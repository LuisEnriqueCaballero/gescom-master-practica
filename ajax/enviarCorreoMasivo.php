...
if (empty($_POST['id_clienteMailMasivo'])) {
    $errors[] = "Correo vacio"; ?>
	<script>
    ...
    <?php }
else if (!empty($_POST['id_clienteMailMasivo'])) {

	$empresa    = 1;
	$tienda     = $_SESSION['tienda'];
	
	$id_factura	= $_POST['id_clienteMailMasivo'];
	//Datos de la factura
	...
	//Datos del cliente
	$sql_cliente 		 = "select * from clientes where cliente_email<>' ' and cliente_email<>'-' and cliente_sucursal='".$tienda."'";
	$query_cliente 		 = mysqli_query($con,$sql_cliente);
	//$row_cliente 		 = mysqli_fetch_array($query_cliente);
	while ($row_cliente  = mysqli_fetch_array($query_cliente, MYSQLI_ASSOC)) {
		$user_email 	 = $row_cliente['cliente_email'];
	}
	//
	include "../mail/enviarCorreoMkt/sendemail1.php";//Mando a llamar la funcion que se encarga de enviar el correo electronico
