...
function sendemail($mail_username,$mail_userpassword,$mail_setFromEmail,$mail_setFromName,$mail_addAddress,$cliente_nombre,$nomempresa,$url1,$url2,$url3,$url4,$url5,$url6,$telefono,$correo,$txt_message,$sistema,$sistema_ruta,$mail_subject,$doc1,$datospdf,$factura_fecha,$moneda1,$factura_ventaTotal,$documento,$folio,$numero,$ruta_pdf,$ruta_xml,$template){
	...
	$message = str_replace('{{pdf}}', $ruta_pdf, $message);
	$message = str_replace('{{xml}}', $ruta_xml, $message);
	...
	$url_xml = $ruta.'/view/pdf/documentos/factura-firmada/'.$doc1;
	$fichero_xml = file_get_contents($url_xml);
	$smtp->addStringAttachment($fichero_xml, $doc1);
