...
$(document).prop('title', '.:: Asistencia | Miracles ::.');
//
...
//Registra entrada
//Registra
$('#guardar_asistenciaModal').submit(function (event) {
  $('#guardar_datos').html(
  ...
  $.ajax({
    type: 'POST',
    url: '../ajax/nuevoAsistenciaEntrada.php',
    data: parametros,
    beforeSend: function (objeto) {},
    success: function (datos) {
      ...
      load(1);
      $('#guardar_asistenciaModal')[0].reset();
      //$("#banco_nombre").focus();
      //console.log(datos);
    },
  });
...
//Registra salida
//Registra
$('#guardar_salidaModal').submit(function (event) {
  $('#guardar_datos').html(
    '<img src="../img/company/load1.svg" style="width: 20px;"> &nbsp; Verificando...'
  );
  $('#guardar_datos').attr('disabled', true);
  ...
  $.ajax({
    type: 'POST',
    url: '../ajax/nuevoAsistenciaSalida.php',
    data: parametros,
    beforeSend: function (objeto) {},
    success: function (datos) {
      $('#resultados_ajax').html(datos);
...
$('#listaAsistencia').addClass('active-link');
